...
test("validateWorkflow は webhook のみの workflow を受理する", () => {
  const workflow = createValidWorkflow();
  workflow.nodes = workflow.nodes.map((node) =>
    node.n8nType === "n8n-nodes-base.manualTrigger"
      ? { ...node, key: "webhookTrigger_1", n8nType: "n8n-nodes-base.webhook" }
      : node,
  );
  workflow.edges = workflow.edges.map((edge) => ({
    ...edge,
    from: edge.from === "manualTrigger_1" ? "webhookTrigger_1" : edge.from,
    to: edge.to === "manualTrigger_1" ? "webhookTrigger_1" : edge.to,
  }));
...
test("validateWorkflow は switch の output index 範囲内なら受理する", () => {
  const workflow = createValidWorkflow();
  workflow.nodes.push({
    key: "switch_8",
  ...
  workflow.nodes.push({ key: "noOp_9", n8nType: "n8n-nodes-base.noOp", typeVersion: 1, parameters: {} });
  workflow.edges.push({
    from: "set_7",
    fromOutputIndex: 0,
    to: "switch_8",
...
test("validateWorkflow は switch の output index 超過を E_INVALID_CONNECTION で返す", () => {
  const workflow = createValidWorkflow();
  workflow.nodes.push({
    key: "switch_8",
    n8nType: "n8n-nodes-base.switch",
  ...
  expect(result.diagnostics).toContainEqual(
    expect.objectContaining({
      ...
      message: expect.stringContaining("switch node"),
    }),
  );
});
