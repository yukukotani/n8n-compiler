...
test("buildControlFlowGraph はパラメータ内の CallExpression を n8n 式に変換する", () => {
  const sourceText = `
    export default workflow({
      name: "sample",
      triggers: [n.manualTrigger()],
...
test("buildControlFlowGraph はパラメータ内の CallExpression でノード参照を解決する", () => {
  const sourceText = `
    export default workflow({
      name: "sample",
      triggers: [n.webhookTrigger()],
...
test("buildControlFlowGraph は execute 内の summarize 呼び出しを受理する", () => {
  const sourceText = `
    export default workflow({
      name: "sample",
      triggers: [n.manualTrigger()],
  ...
  expect(statement?.type).toBe("NodeCall");
  if (statement?.type === "NodeCall") {
    expect(statement.call.kind).toBe("summarize");
    expect(statement.call.parameters).toEqual({ fieldsToSummarize: ["content"] });
  }
});
...
test("buildControlFlowGraph は execute 内の code 呼び出しを受理し params を保持する", () => {
  ...
  expect(statement?.type).toBe("NodeCall");
  if (statement?.type === "NodeCall") {
    expect(statement.call.kind).toBe("code");
    expect(statement.call.parameters).toEqual({
      jsCode: "return items;",
...
test("buildControlFlowGraph は execute 内の executeWorkflow 呼び出しを受理し params を保持する", () => {
  const sourceText = `
    export default workflow({
      name: "sample",
      triggers: [n.manualTrigger()],
