...
test("executeWorkflow を ActionNode として定義でき、params をそのまま保持する", () => {
  const node = n.executeWorkflow({
    workflowId: "wf_123",
    mode: "once",
  ...
  expect(node.__brand).toBe("NodeRef");
  expect(node.kind).toBe("executeWorkflow");
  ...
  expect(definition.name).toBe("execute-workflow");
});
